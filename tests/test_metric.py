...
async def test_create_metric(api_client):
    metric_data = {"service_name": "TestService", "path": "/test", "response_time_ms": 100}
...
async def test_get_metrics(api_client, db):
    await create_metric(db, service_name="ServiceA", path="/path1", response_time_ms=100)
    await create_metric(db, service_name="ServiceA", path="/path1", response_time_ms=250)
    await create_metric(db, service_name="ServiceA", path="/path1", response_time_ms=250)

    await create_metric(db, service_name="ServiceB", path="/path3", response_time_ms=150)
