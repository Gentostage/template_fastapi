...
from app.schemas.metric import MetricCreate, MetricServiceResponse, MetricResponse

router = APIRouter()

@router.post("/metrics", response_model=MetricResponse)
async def add_metric(metric_data: MetricCreate, session: AsyncSession = Depends(get_async_session)):
    return await create_metric(session, path=metric_data.path, response_time_ms=metric_data.response_time_ms, service_name=metric_data.service_name )
